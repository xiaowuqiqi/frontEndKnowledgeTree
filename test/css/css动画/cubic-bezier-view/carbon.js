var _carbonads = {
  ...
  init: function(where, force_serve) {
    ...
    if (forceUrlPreview) {
      ...
      var jsonSrc = this._buildSrvUrl(protocol + "//" + baseurl + "/ads/" + serve + ".json?segment=placement:" + placement);
      this._loadJson(jsonSrc, _carbonads_go)
    }
  },
...
function _carbonads_go(b) {
  if (!b)
    return;
  var ad = b["ads"][0], link, fulllink, image, description, time = Math.round(Date.now() / 1e4) | 0;
  ...
  if (typeof link[1] != "undefined" && _carbonads.isset(ad.statlink))
    ...
    fulllink = link;
  fulllink = fulllink.replace(/srv.buysellads.com/g, _carbonads.srv());
